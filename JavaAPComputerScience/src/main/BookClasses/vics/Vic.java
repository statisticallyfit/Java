...
public class Vic
{
	...
	private int itsPos;             // 0,1,2,...; 0 is at front - zero-based
...
/* ACTION METHODS */
	...
	{	if (itsPos == 0)
        ...
        //else if itsPos != 0 then decrement itsPos and back up
		itsPos--;
	...
	/** Terminate the program with an appropriate showUpdatedStatus.  */
...
/* METHODS THAT USE THE FRAME ---- THESE ARE CLASS METHODS*/
...
/* THE INITIALIZER AND CONSTRUCTOR METHODS */
	...
	{	super();
		itsSequence =  theNumVics < theMaxVics
				?   theSeq[theNumVics]   :  new ArrayList(); //todo: themaxvics doesn't get incremented properly??? -- new theSeq of size 0
		itsPos = 0;
		itsID = theNumVics + 1;
...
// THE NESTED FRAME CLASS
	...
	{	private final int SLOT = 75;           // between CD slots
		...
		{	addWindowListener (new Closer());
			...
			setBackground (new Color (233, 233, 255)); // a nice cream --- CHANGED TO LAVENDER
		...
		{	// PRINT THE vicSay MESSAGE AT THE TOP
			...
			page.setColor (new Color (30, 65, 217));  // a light green---- CHANGED TO blue
			page.drawString (vicSay, 25, SAY - 5); // showUpdatedStatus

			// DRAW UP TO FOUR Vic SEQUENCES AND THE STACK
			for (int k = 0;  k < theMaxVics;  k++)
				drawSequence (page, k, TOPSEQ + k * DIST);
			page.setColor (Color.blue);//was red
		...
		{	// <x, y> is the lower-left corner of the stick figure
			...
			page.setColor (Color.black);//was black
